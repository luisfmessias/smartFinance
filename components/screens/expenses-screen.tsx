...
export function ExpensesScreen({ expenses, onAddExpense }: { expenses: Expense[]; onAddExpense: () => void }) {
  ...
  return (
    <div className="animate-in">
      ...
      <div className="mt-5 flex gap-2"><label className="flex flex-1 items-center gap-2 rounded-2xl bg-white px-4 py-3 shadow-sm"><Search size={17} className="text-slate-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar gasto" className="min-w-0 flex-1 bg-transparent text-sm outline-none" /></label><button onClick={() => setShowFilters(true)} className={`rounded-2xl p-3 shadow-sm ${category === "all" ? "bg-white" : "bg-mint text-leaf"}`}><SlidersHorizontal size={18} /></button></div>
      {filtered.length ? <div className="mt-5 space-y-2">{filtered.map((expense) => <ExpenseRow key={expense.id} expense={expense} />)}</div> : <div className="mt-5 rounded-2xl border border-dashed border-slate-200 bg-white p-5 text-center"><p className="text-sm font-bold">{expenses.length ? "Nenhum gasto encontrado" : "Você ainda não registrou gastos"}</p><p className="mt-2 text-xs leading-5 text-slate-500">{expenses.length ? "Tente ajustar a busca ou remover os filtros." : "Adicione seu primeiro lançamento para começar."}</p>{!expenses.length && <button onClick={onAddExpense} className="mt-4 rounded-xl bg-forest px-4 py-2.5 text-xs font-bold text-white">Adicionar gasto</button>}</div>}
